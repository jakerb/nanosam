...
def gateway(
    ...
    if config.voice.enabled:
        try:
            from nanobot.voice import VoiceAssistantService, VoiceDependencyError
            voice_service = VoiceAssistantService(config, agent)
            console.print(f"[green]✓[/green] Voice: wake word '{config.voice.wake_word}'")
        except VoiceDependencyError as e:
            console.print(f"[red]Voice disabled:[/red] {e}")
    ...
    async def run():
        try:
            ...
            if voice_service:
                tasks.append(voice_service.start())
...
@app.command()
def voice():
    """Run the local voice assistant (wake word mode)."""
    from nanobot.config.loader import load_config
    from nanobot.bus.queue import MessageBus
    from nanobot.agent.loop import AgentLoop
    from nanobot.voice import VoiceAssistantService, VoiceDependencyError
    ...
    try:
        voice_service = VoiceAssistantService(config, agent_loop)
    except VoiceDependencyError as e:
        console.print(f"[red]Voice disabled:[/red] {e}")
        raise typer.Exit(1)

    asyncio.run(voice_service.start())
